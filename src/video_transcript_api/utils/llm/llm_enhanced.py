...
class EnhancedLLMProcessor:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.enable_risk_model_selection = self.llm_config.get(
            "enable_risk_model_selection", False
        )

        # 结构化校对的校验模型配置
        calibration_config = self.llm_config.get("structured_calibration", {})
        self.validator_model = calibration_config.get(
            "validator_model", self.calibrate_model
        )
        ...
        if self.enable_risk_model_selection:
            if not self.risk_summary_model:
                logger.error(
                    "enable_risk_model_selection is True but risk_summary_model is not configured"
                )
                raise ValueError(
                    "risk_summary_model must be configured when enable_risk_model_selection is enabled"
                )
            logger.info(
            ...
            logger.info(
                f"Risk model selection disabled. Using default models: calibrate={self.calibrate_model}, summary={self.summary_model}"
            )
    ...
    @staticmethod
    def _format_transcript_for_display(
        transcript: str, sentences_per_paragraph: int = 3
    ) -> str:
        """
    ...
    def _detect_risk_in_metadata(
        ...
        try:
            ...
            from video_transcript_api.utils.risk_control import (
                is_enabled,
                sanitize_text,
            )
            ...
            if not is_enabled():
                self.logger.debug(
                    "Risk control module is not enabled, skipping risk detection in metadata"
                )
                return False, []
    ...
    def _select_models(
        self, task_id: str, title: str, author: str, description: str
    ) -> dict:
        """
        ...
        # 一次检测，三处使用
        has_risk, sensitive_words = self._detect_risk_in_metadata(
        ...
        if has_risk:
            ...
            if self.risk_calibrate_model:
                ...
                result["calibrate_reasoning_effort"] = (
                    self.risk_calibrate_reasoning_effort
                )

            # 校验模型：如果配置了风险校验模型则切换，否则保持默认
            if self.risk_validator_model:
                result["validator_model"] = self.risk_validator_model
                result["validator_reasoning_effort"] = (
                    self.risk_validator_reasoning_effort
                )
    ...
    def process_llm_task(self, llm_task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if use_speaker_recognition and transcription_data and platform and media_id:
            logger.info(
                f"检测到说话人识别场景，使用结构化处理: {task_id}, 平台: {platform}, 媒体ID: {media_id}"
            )
            result = self._process_with_structured_output(
                llm_task,
                selected_summary_model,
                selected_summary_effort,
                selected_calibrate_model,
                selected_calibrate_effort,
                selected_validator_model,
                selected_validator_effort,
            )
        elif use_speaker_recognition and transcription_data:
            # FunASR JSON格式，需要创建临时文件进行分段处理
            text_length = len(transcript)
            need_segmentation = (
                text_length > self.segmentation_processor.enable_threshold
            )
    ...
    def _process_with_structured_output(
        ...
        selected_validator_model: str = None,
        selected_validator_effort: str = None,
    ) -> Dict[str, Any]:
        """
        使用结构化输出处理说话人识别任务
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
                ...
                try:
                    ...
                    logger.error(
                        f"TXT分段校对线程异常: {task_id}, 错误: {exc}", exc_info=True
                    )
                ...
                try:
                    ...
                    logger.error(
                        f"TXT分段总结线程异常: {task_id}, 错误: {exc}", exc_info=True
                    )
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
                ...
                try:
                    ...
                    logger.error(
                        f"JSON分段校对线程异常: {task_id}, 错误: {exc}", exc_info=True
                    )
                    calibrated_text = f"【LLM call failed】Thread exception: {exc}"
                try:
                    summary_text = summary_future.result()
                    ...
                    logger.error(
                        f"JSON分段总结线程异常: {task_id}, 错误: {exc}", exc_info=True
                    )
                    summary_text = f"【LLM call failed】Thread exception: {exc}"
    ...
    @staticmethod
    def _build_json_summary_text(
        transcription_data: Dict[str, Any], speaker_mapping: Dict[str, str]
    ) -> str:
    ...
    def _process_original_logic(
        ...
        logger.info(
            f"使用原有逻辑处理文本: {task_id}, 文本长度: {original_length}, 阈值: {min_summary_threshold}"
        )
        ...
        def run_calibrate():
            try:
                ...
                if not calibrated or not calibrated.strip():
                    ...
                    result_dict["校对文本"] = (
                        f"【LLM call failed】Empty response from calibrate API"
                    )
                else:
        ...
        def run_summary():
            try:
                logger.info(
                    "短文本总结任务开始: %s, 模型: %s", task_id, selected_summary_model
                )
                # 检测说话人数量以选择 system prompt（KV Cache 优化）
                speaker_count = self._detect_speaker_count(
                    use_speaker_recognition, transcription_data, transcript
                )
                ...
                if not summary or not summary.strip():
                    ...
                    result_dict["内容总结"] = (
                        f"【LLM call failed】Empty response from summary API"
                    )
                else:
        ...
        if original_length >= min_summary_threshold:
            ...
            skip_summary = False
            logger.info(
                f"文本长度 {original_length} >= {min_summary_threshold}，执行总结"
            )
        else:
            # 文本太短，跳过总结
            t1.join()
    ...
    def _ensure_min_length(
        self, original: str, calibrated: str, task_id: str, context: str
    ) -> str:
    ...
    def _generate_original_calibrate_prompt(
        self,
        transcript: str,
        video_title: str,
        author: str,
    ...
    def _generate_original_summary_prompt(
        self,
        transcript: str,
        video_title: str,
        author: str,
...
"""
    ...
    def _detect_speaker_count(
        ...
        if transcription_data:
            ...
            logger.debug(
                f"FunASR 转录结果：从 transcription_data 检测到 {count} 个说话人"
            )
            return count
    ...
    def _build_context_info(
        self, video_title: str, author: str, description: str
    ) -> str:
        """构建辅助信息（完全共用）"""
...
"""
    ...
    def process_llm_task_with_structure(
        ...
        try:
            ...
            original_speakers = mapping_inference.extract_speakers_from_funasr(
                funasr_data
            )
            ...
            # 2. 生成说话人推断提示词
            speaker_inference_prompt = self._generate_speaker_inference_prompt(
                funasr_data, original_speakers, video_metadata
            )
            ...
            # 5. 处理说话人映射结果
            speaker_mapping = self._process_speaker_mapping_result(
                speaker_mapping_result, original_speakers
            )

            # 6. 提取带时间信息的对话数据
            logger.info("提取带时间信息的对话数据")
            dialogs_with_time = (
                StructuredCalibrator.extract_time_enhanced_dialogs_from_funasr(
                    funasr_data, speaker_mapping
                )
            )
            ...
            logger.info("开始结构化校对")
            calibrated_dialogs = (
                self.structured_calibrator.calibrate_structured_dialogs(
                    dialogs_with_time,
                    video_metadata,
                    selected_calibrate_model=selected_calibrate_model,
                    selected_calibrate_effort=selected_calibrate_effort,
            ...
            # 7. 生成兼容性文本版本
            calibrated_text = self._generate_text_from_calibrated_dialogs(
                calibrated_dialogs
            )

            # 8. 生成总结（检查是否可以复用已有结果）
            summary_text = self._get_or_generate_summary(
                cache_dir,
                calibrated_text,
                video_metadata,
                speaker_mapping,
            ...
            # 10. 保存结果到缓存
            self._save_structured_result(
                cache_dir, structured_result, calibrated_text, summary_text
            )
    ...
    def _generate_speaker_inference_prompt(
        self, funasr_data: Dict, original_speakers: List[str], video_metadata: Dict
    ) -> str:
        """生成说话人推断提示词"""
        video_title = video_metadata.get("video_title", "未知标题")
        author = video_metadata.get("author", "未知作者")
...
"""
    ...
    def _extract_context_snippets(
        self, funasr_data: Dict, speakers: List[str], max_snippets: int = 10
    ) -> str:
        """提取关键的转录片段作为推断上下文"""
        snippets = []
    ...
    def _process_speaker_mapping_result(
        self, result: StructuredResult, original_speakers: List[str]
    ) -> Dict[str, str]:
        """
        处理说话人映射结果
    ...
    def _generate_structured_dialogs(
        self, funasr_data: Dict, speaker_mapping: Dict[str, str]
    ) -> List[Dict[str, str]]:
        """基于映射关系生成结构化对话"""
        dialogs = []
        ...
        if current_speaker and current_content:
            dialogs.append(
                {"speaker": current_speaker, "content": " ".join(current_content)}
            )

        return dialogs

    def _generate_text_from_structured_dialogs(
    ...
    def _generate_text_from_calibrated_dialogs(
        self, calibrated_dialogs: List[Dict[str, Any]]
    ) -> str:
        """从校对后的对话生成文本版本"""
        text_lines = []
    ...
    def _get_or_generate_summary(
        ...
        # 生成新的总结
        logger.info("生成新的总结")
        return self._generate_summary_from_structured_content(
            calibrated_text,
            video_metadata,
            speaker_mapping,
            selected_summary_model,
    ...
    def _generate_summary_from_structured_content(
        self,
        calibrated_text: str,
        video_metadata: Dict,
        speaker_mapping: Dict[str, str],
        ...
        transcription_data = {"speakers": speakers} if speakers else None
